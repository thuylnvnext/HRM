...
namespace Common
{
    ...
    public class Reader
    {
        ...
        private static CZKEMClass reader;
        ...
        public LogDataList GetLogData()
        {
            ...
            bool error = false;
            while (endData != "FFFFFFFFFFFFFFFFFFFFFF")
            {
                ...
                for (int i = 0; i < logLen; i++)
                {
                    ...
                    if (endData == "FFFFFFFFFFFFFFFFFFFFFF") break;
        ...
        public static bool AddCard(int enrollNumber, string cardID, string name)
        {
            bool retVal = true;
            if (reader == null) reader = new CZKEMClass();
            ReaderTable _reader = new ReaderTable();
            _reader.GetContent();

            foreach (ReaderRow rr in _reader)
            {
                if (reader.Connect_Net(rr.IPAddress, Convert.ToInt32(rr.CommPort)))
                {
                    reader.EnableDevice(1, false);
                    reader.SetStrCardNumber(cardID);
                    if (!reader.SetUserInfo(1, enrollNumber, name, string.Empty, 0, true))
                    {
                        retVal = false;
                        break;
                    }
                    reader.RefreshData(1);
                    reader.EnableDevice(1, true);
                }
                reader.Disconnect();
            }
        ...
        public static bool AddCard(int enrollNumber, string cardID, string name, System.Data.SqlClient.SqlConnection conn, System.Data.SqlClient.SqlTransaction tran)
        {
            bool retVal = true;
            if (reader == null) reader = new CZKEMClass();
            ReaderTable _reader = new ReaderTable();
            _reader.GetContent(conn, tran);
        ...
        public static bool AddCard(int enrollNumber1, int enrollNumber2, string cardID, string name)
        {
            bool retVal = true;
            CZKEMClass reader = new CZKEMClass();
            ReaderTable _reader = new ReaderTable();
            _reader.GetContent();
            ...
            foreach (ReaderRow rr in _reader)
            {
                ...
                {
                    reader.EnableDevice(1, false);
                    if (!reader.DeleteEnrollData(1, enrollNumber1, 1, 12))
                    {
                        retVal = false;
                        break;
                    }
                    reader.SetStrCardNumber(cardID);
                    if (!reader.SetUserInfo(1, enrollNumber2, name, string.Empty, 0, true))
                    {
                        retVal = false;
                        break;
                    }
        ...
        public static bool DeleteCard(int enrollNumber)
        {
            bool retVal = true;
            CZKEMClass reader = new CZKEMClass();
            ReaderTable _reader = new ReaderTable();
            ...
            foreach (ReaderRow rr in _reader)
            {
                ...
                {
                    reader.EnableDevice(1, false);
                    if (!reader.DeleteEnrollData(1, enrollNumber, 1, 12))
                    {
                        retVal = false;
                        break;
                    }
        ...
        public static bool DeleteCard(int enrollNumber, System.Data.SqlClient.SqlConnection conn, System.Data.SqlClient.SqlTransaction tran)
        {
            bool retVal = true;
            CZKEMClass reader = new CZKEMClass();
            ReaderTable _reader = new ReaderTable();
